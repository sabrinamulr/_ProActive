    ...
    public class AppDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Projekt>(p =>
            {
                ...
                p.HasIndex(x => new { x.Status, x.Phase }); //Mit enum
                //Für ohne enum phasen bei seed daten 
                //p.HasIndex(x => new { x.Status });
                //p.HasIndex(x => x.Phase);
            });
...
//                // Copilot: Relation PhaseDefinition is inferred by FK PhaseId -> Phase// NEIN
//                p.HasIndex(x => x.BenutzerId);
//                p.HasIndex(x => x.ProjektleiterId);
//                p.HasIndex(x => x.AuftraggeberId);
//                p.HasIndex(x => x.Phase);
...
//                // Use the entity properties ProjektId / PhaseId / VerantwortlicherBenutzerId
//                pp.HasOne(x => x.Projekt)
//                    .WithMany(p => p.ProjektPhasen)
//                    .HasForeignKey(x => x.ProjektId)
//                    .OnDelete(DeleteBehavior.Cascade);
...
//                pp.HasIndex(x => x.ProjektId);
//                pp.HasIndex(x => x.PhaseId);
//                pp.HasIndex(x => x.VerantwortlicherBenutzerId);
//            });
